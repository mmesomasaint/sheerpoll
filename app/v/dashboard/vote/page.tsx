...
export default function Timeline() {
  ...
  const totalVotes = useMemo(() => position?.candidates.reduce(
      ...
      <div className='flex justify-between items-center py-5 gap-5'>
        ...
        <p className='text-4xl font-bold uppercase'>Vice President</p>
          ...
          <div className='flex flex-col items-end gap-0'>
            ...
            <p className='text-xl text-center font-semibold uppercase w-full py-1 border border-primary text-primary rounded-md'>534</p>
          </div>
        </div>
      <div className='grow flex flex-col w-full'>
        <div className='flex justify-center items-center gap-10 pb-1 border-b border-b-primary/60'>
          <div className='relative'>
            ...
            <div
              className='absolute -bottom-[0.415rem] w-full border-2 border-primary'
            />
          </div>
        </div>
    <div className='grow flex px-20 flex-col justify-start w-full'>
    <CandidateCard name={'Obi'} votes={45} active={false} onVoteClick={() => setChoice('abc')} />
          <button
            type='submit'
            disabled={!choice}
  ...
  return (
    ...
    <div className='grid grid-cols-3 gap-10 place-items-center'>
      ...
      <div className='flex flex-col items-center gap-0'>
        ...
        <div className={`w-10 h-10 rounded-sm border-4 ${active ? 'border-primary' : 'border-black/60'} flex justify-center items-center shadow-sm`} onClick={onVoteClick}>
