...
function CreaetePositionForm({title, setTitle}: {title: string, setTitle: (title: string) => void}) {
...
function Candidates({candidates, setCandidates}: {candidates: CandidateType[], setCandidates: (candidate: object) => void}) {
  ...
  return (
    ...
    <input
      ...
      placeholder='title'
      className='border border-gray-400 rounded-md px-3 py-2 w-full'
      onChange={(e) => setCandidate(pre => ({...pre, ['name']: e.target.value}))}
    />
  </div>
    <button
      type='button'
